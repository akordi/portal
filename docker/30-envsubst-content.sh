...
for line in $(ls -1p /usr/share/nginx/html/ | grep -v /); do
  sed -i "s|{{APP_DESCRIPTION}}|${APP_DESCRIPTION:-""}|g" /usr/share/nginx/html/${line}
  sed -i "s|{{APP_NAME}}|${APP_NAME:-"Akordi"}|g" /usr/share/nginx/html/${line}
  sed -i "s|{{APP_TITLE}}|${APP_TITLE:-"Akordi"}|g" /usr/share/nginx/html/${line}
  sed -i "s|{{AUTH_URL}}|${AUTH_URL:-""}|g" /usr/share/nginx/html/${line}
